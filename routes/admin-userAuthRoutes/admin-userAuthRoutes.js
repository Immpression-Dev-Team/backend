...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: 'Admin logged in', token, email: admin.email });
  } catch (error) {
  ...
  try {
    ...
    if (page <= 0) {
      return res.status(400).json({
        error: 'Invalid page number. Please provide a positive integer.',
      });
    }
    ...
    const images = await ImageModel.find(query).skip(skip).limit(limit).exec();
...
router.get('/all_images/stats', isAdminAuthorized, async (_, res) => {
  try {
    ...
    const approvedCount = await ImageModel.countDocuments({
      stage: 'approved',
    });
    const rejectedCount = await ImageModel.countDocuments({
      stage: 'rejected',
    });
...
router.put('/art/:id/approve', isAdminAuthorized, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: 'Artwork approved', art: updatedArt });
  } catch (error) {
...
router.get('/users', isAdminAuthorized, async (req, res) => {
  try {
    const users = await UserModel.find(
      {},
      'name email role createdAt profilePictureLink'
    );
    ...
    return res
      .status(200)
      .json({ success: true, totalUsers: users.length, users });
  } catch (error) {
...
router.delete('/art/:id', isAdminAuthorized, async (req, res) => {
  try {
    ...
    try {
      ...
      console.log(
        `🗑️ Cloudinary deletion result for ${publicId}:`,
        cloudinaryResult
      );
    } catch (cloudError) {
...
router.delete('/user/:id', isAdminAuthorized, async (req, res) => {
  try {
    ...
    if (publicIdsToDelete.length > 0) {
      try {
        cloudinaryResult = await cloudinary.v2.api.delete_resources(
          publicIdsToDelete,
          {
            type: 'upload',
            resource_type: 'image',
        ...
        console.log(
          `🗑️ Cloudinary deletion result for user's artwork:`,
          cloudinaryResult
        );
      } catch (cloudError) {
        console.error(
          'Cloudinary deletion error (continuing with other deletions):',
          cloudError
        );
        // Handle Cloudinary API errors gracefully (log, skip, and continue)
      }
    ...
    if (user.profilePictureLink) {
      ...
      try {
        const profilePicResult = await cloudinary.v2.api.delete_resources(
          [profilePicPublicId],
          {
            type: 'upload',
            resource_type: 'image',
        ...
        console.log(
          `🗑️ Cloudinary deletion result for profile picture ${profilePicPublicId}:`,
          profilePicResult
        );
      } catch (cloudError) {
        console.error(
          'Cloudinary deletion error for profile picture (continuing):',
          cloudError
        );
    ...
    res.status(200).json({
      success: true,
      message:
        'User, profile picture, and all associated artwork deleted successfully',
